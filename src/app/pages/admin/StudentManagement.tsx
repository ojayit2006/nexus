...
export function StudentManagement() {
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    e.preventDefault();
    if(formData.name && formData.email && formData.enum) {
      ...
      const payload = {
        ...
        role: 'student',
        password: formData.pass // Although students use UID as password, we pass this for future flexibility or if needed by backend
      };

      const result = await register(payload);

      if (result.success) {
        // 2. Insert them into the Admin's frontend display table for immediate feedback
        addStudent(formData);
        setModalOpen(false);
        setFormData({ name: '', enum: '', branch: 'CSE', year: '2025', phone: '', email: '', pass: '' });
        triggerToast('Student account created and saved to database.');
      } else {
